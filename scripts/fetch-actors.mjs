// Bake OMDb's Actors field (top 3-4 billed, comma-separated) for every film
// into src/data/actors.json so the cast line renders instantly — no
// dependence on each user's local OMDb cache.
...
const OUT_PATH    = path.resolve(__dirname, '../src/data/actors.json');
...
async function main() {
  ...
  console.log(`Fetching Actors from OMDb for ${MOVIES.length} films...`);

  const out = {};
  let longest = { id: null, title: null, year: null, actors: '', len: 0 };
  for (let i = 0; i < MOVIES.length; i++) {
    ...
    if (actors) {
      out[m.id] = actors;
      if (actors.length > longest.len) {
        longest = { id: m.id, title: m.title, year: m.year, actors, len: actors.length };
      }
    }
  ...
  console.log(`\n\n${Object.keys(out).length} films with cast data → ${OUT_PATH}`);
  console.log(`\nLongest cast string (${longest.len} chars):`);
  console.log(`  ${longest.title} (${longest.year})`);
  console.log(`  ${longest.actors}`);
}
